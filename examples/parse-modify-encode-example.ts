...
import { OBX, OBR, MSH, PID } from "../src/segments/v2.5.1";
...
console.log(`Number of patients: ${parseResult.data.patientResults.length}`);
console.log(
  `Number of orders for patient 1: ${parseResult.data.patientResults[0].orderObservations.length}`,
);
console.log("\n");

// Extract the data for modification
const msh = parseResult.data.msh;
const patientResult = parseResult.data.patientResults[0]; // First (and only) patient
...
if (!reParseResult.success || !reParseResult.data) {
  console.error("Failed to re-parse modified message:", reParseResult.error);
  process.exit(1);
}

console.log("✓ Re-parsed successfully!");
console.log(
  `  Patients: ${reParseResult.data.patientResults.length}`,
);
console.log(
  `  Orders: ${reParseResult.data.patientResults[0].orderObservations.length}`,
);
console.log(
  `  CBC OBX count: ${reParseResult.data.patientResults[0].orderObservations[2].obxList.length}`,
);
console.log("\n");
