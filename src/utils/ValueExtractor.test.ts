...
import { ValueExtractor, extractValue } from "./ValueExtractor.ts";
...
PID|1||12345^^^GEN&2.16.840.1.113883.3.239&ISO^MR||Doe^John^Q^Jr^Dr||19800115|M|||123 Main St^Apt 4^Springfield^IL^62701^USA||555-1234|555-5678
...
// PID-3 = 12345^^^GEN&2.16.840.1.113883.3.239&ISO^MR
// CX.4 (assigning authority) is an HD: namespace&universalId&universalIdType
test("ValueExtractor extracts subcomponent 1 from assigning authority", () => {
  const extractor = new ValueExtractor();
  expect(extractor.get("PID-3.4.1", sampleMessage)).toBe("GEN");
});

test("ValueExtractor extracts subcomponent 2 from assigning authority", () => {
  const extractor = new ValueExtractor();
  expect(extractor.get("PID-3.4.2", sampleMessage)).toBe(
    "2.16.840.1.113883.3.239",
  );
});

test("ValueExtractor extracts subcomponent 3 from assigning authority", () => {
  const extractor = new ValueExtractor();
  expect(extractor.get("PID-3.4.3", sampleMessage)).toBe("ISO");
});

test("ValueExtractor returns null for out-of-bounds subcomponent", () => {
  const extractor = new ValueExtractor();
  expect(extractor.get("PID-3.4.99", sampleMessage)).toBe(null);
});

test("ValueExtractor returns full component when no subcomponent index given", () => {
  const extractor = new ValueExtractor();
  expect(extractor.get("PID-3.4", sampleMessage)).toBe(
    "GEN&2.16.840.1.113883.3.239&ISO",
  );
});
